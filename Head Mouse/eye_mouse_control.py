...
def calibrate_blink_threshold(landmarks, calibration_frames=30):
    """Calibrate blink threshold based on user's normal eye state."""
    differences = []
...
def detect_blink(landmarks):
    """Detects left eye blinks and triggers a click."""
...
def main():
    ...
    while True:
        ...
        if landmark_points:
            ...
            # Detect Blink (Left Eye for Click)
            detect_blink(landmarks)

            # Detect Eye Movement (Right Eye for Mouse Control)
        ...
        cv2.putText(frame, "Eye Mouse (Right Eye) + Blink Click (Left Eye)", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
